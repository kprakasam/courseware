...
CREATE TABLE `assignment` (
  ...
  `external_tool_id` int(11) DEFAULT NULL COMMENT 'External Tool that launches this assignment',
  ...
  `canvas_lti_assignment_name` varchar(255) NOT NULL COMMENT ' External tool name Maps to resource_link_title parameter of POST request',
  ...
  CONSTRAINT `FK_EXTERNAL_TOOL_ID` FOREIGN KEY (`external_tool_id`) REFERENCES `external_tool` (`id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COMMENT='Holds Assignment Details'$$
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COMMENT='Holds Course Details'$$
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COMMENT='Holds Jars for a given Assignment'$$
